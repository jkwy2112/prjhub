...
| BPMN 审批流 | 基于 **SpiffWorkflow**( Python BPMN 2.0 引擎 ): 多级审批链、条件网关自动路由、会签/或签/票签、流程定义版本化、审批单中途持久化、审批中心( 待办/时间线/撤回/打印 ) |
| 可视化流程设计器 | WFlow 风格嵌套树: **表单设计器**( 9 种控件/字段ID/必填/选项 ) + 流程设计( 审批人[固定/发起时指定/表单联系人]、条件分支[表单字段类型感知比较符]、并行分支、抄送人、触发器[Webhook]、会签/或签/票签、审批人为空策略、驳回退回上一节点、超时催办、实时校验错误面板+卡片标红 ) |
...
- 催办调度器: 默认每 10 分钟扫描超时审批并推送企业微信 ( `REMINDER_ENABLED=false` 可关闭 )
- 数据库迁移: 启动时自动补齐新增列 ( SQLite / PostgreSQL )
